...
les chemins, les outils de sauvegarde/restauration, les options et l’essai VPN,
le récapitulatif, la progression, les indexeurs et le rapport sont testables.
...
Pour un essai qui agit réellement sur Docker, double-cliquer sur
**TESTER-INSTALLATION-REELLE.cmd**. Ce lanceur force l’assistant web sans
`--demo` et place les fichiers du projet dans le sous-dossier distinct
`installation-test-reelle`. Le terminal affiche **MODE REEL** et la page ne
porte pas le bandeau « Mode démonstration ».

Dans l’assistant, choisir également un nom de pile, un `CONFIG_ROOT` et un
`DATA_ROOT` réservés au test. Après la confirmation finale, les conteneurs et
les fichiers indiqués seront réellement créés ou modifiés. Docker Desktop doit
donc être démarré.
...
- Chemins et profil de plateforme, identifiant, hôte, langues, PUID/PGID et test
  manuel de création de lien physique.
- VPN Gluetun pour les clients BitTorrent et, sur choix explicite, pour
  SABnzbd ; connexion directe + SSL/TLS recommandée pour SABnzbd, fournisseurs
  compatibles avec la redirection de port placés en premier, localisations
  multiples filtrables et essai par tunnel jetable.
- Sauvegarde complète d’une pile et restauration après lecture du manifeste et
  confirmation explicite. Une sauvegarde sûre arrête les conteneurs par défaut.
...
- Après installation, ajout facultatif d’indexeurs à partir des définitions du
  Prowlarr installé, puis rapport des URL, identifiants, clés API et prochaines
  étapes. La page d’accès et la console d’administration restent ouvrables.
...
- Reprise d’une pile ou nouvelle configuration, au choix : réglages disponibles,
  secrets, ports et images sont conservés seulement en mode reprise. Les services
  peuvent être décochés. Une configuration incompatible n’est effacée qu’après
  un second choix explicite au récapitulatif.
...
L’interface possède des libellés FR/EN. Certains messages remontés par le moteur
restent dans la langue active du processus ; la traduction de chaque erreur
externe n’est pas revendiquée.
...
## Parité avec le TUI — 11 septembre 2026

Le parcours web couvre maintenant les outils qui restaient propres au TUI :
diagnostic Docker, sauvegarde, inspection puis restauration, informations
PUID/PGID et test des chemins, sélection VPN multi-lieux et test jetable,
reprise/remise à zéro d’une pile, rapport complet, page d’accès et gestion des
indexeurs Prowlarr. La matrice vérifiable est dans `docs/WEB_TUI_PARITY.md`.

La validation complète de cet état compte **1 168 tests réussis, 5 ignorés**.
Ruff, la syntaxe JavaScript, le contrat de fin de parcours, le smoke test TUI et
la reconstruction du paquet Python réussissent. L’exécutable Windows publié a
été identifié et inspecté comme archive PyInstaller, mais il n’a pas pu être
lancé nativement dans cet environnement Linux ; le rendu visuel du nouveau
parcours doit également être confirmé sur la machine Windows cible.

Le correctif local `3ef1375` est intégré : SABnzbd utilise le port interne 8085.
En trajet direct, il publie lui-même `8085:8085` ; dans la pile réseau Gluetun,
Gluetun publie ce port et la liste blanche de SABnzbd accepte son nom. Les
anciennes configurations 0.8.0 sont migrées sans changer leur trajet. Le port
entrant VPN n’est plus attribué à deux clients partageant le même tunnel.
