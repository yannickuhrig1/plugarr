...
- `plugarr\TESTER-INSTALLATION-REELLE.cmd` ouvre le véritable assistant web :
  après confirmation, ses actions modifient réellement Docker et les dossiers
  choisis. Il utilise `plugarr\installation-test-reelle` pour les fichiers du
  projet ; choisir aussi des racines de configuration/données et un nom de pile
  dédiés au test.
...
Avec qBittorrent sous VPN et le choix SABnzbd recommandé, Gluetun publie
`8080:8080`, SABnzbd publie lui-même `8085:8085`, et le câblage vise
`http://sabnzbd:8085`. Si « Faire aussi passer SABnzbd par le VPN » est choisi,
Gluetun publie également `8085:8085` et le câblage vise
`http://gluetun:8085`. Si qBittorrent et Transmission sont tous les deux
sélectionnés avec un fournisseur compatible, seul qBittorrent reçoit le port
entrant ; Transmission reste protégé par le tunnel sans port entrant dédié.
