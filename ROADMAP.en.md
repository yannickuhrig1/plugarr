...
**Last updated: 13 September 2026** — published version: **0.9.0**
...
and no warning. `scripts/audit_traductions.py` therefore collects all 639
...
| **Local web wizard** | ✅ shipped in 0.9.0 | The terminal's full journey, in the browser: backup and restore, paths, VPN and tunnel test, quality profiles, summary, indexers after installation. The connection map shows what will be wired, then each link live. Server on `127.0.0.1` behind a session token. Exercised in a real browser and through a real end-to-end installation on Windows: 6 services, 18 links out of 18. |
| **Maintenance console** | ✅ shipped in 0.9.0 | Scheduled backups with rotation, history, alerts, testing and repairing one specific link, automatic update of `plugarr.exe` checked against its SHA-256 digest. |
...
**Shipped in 0.9.0.** Verified in CI against a real Sonarr,
...
| Version | |
|---|---|
| **0.9.0** | **The preflight froze for more than ten minutes on the default path.** `check_writable` probed with `tempfile.NamedTemporaryFile`, which on Windows catches `PermissionError` and retries up to ten thousand times whenever `os.access` says yes — which it wrongly does for `C:\`. And `C:/plugarr/data` has the drive root as its first existing ancestor on the very first installation. Once instant, the probe then refused that same path: a standard account cannot create a **file** at the root, but creates the **folder** just fine. The check now reproduces the real sequence of operations. **Found because the test suite itself no longer returned.** |
| **0.9.0** | **The console froze during a backup.** `do_GET` and `do_POST` ran under the lock `backup()` held for the whole archiving. Two locks now: state in short holds, long operations apart. Measured on a real stack: `/api/status` in 574 ms against 9,935 ms during a 49.8 MB backup. |
| **0.9.0** | **Every backup raised six false alerts.** A cold backup stops the stack; the status poll saw the stopped containers and cried "unavailable" about the operation the console had just started. Six alerts and twelve events per backup, against zero. |
| **0.9.0** | **Two torrent clients split the downloads in alternation.** All were declared at `priority: 1`, and Sonarr then applies round-robin. The preferred client goes to 1, the others stay as fallback; a manual setting is never overwritten. Verified in CI against a real Sonarr. |
| **0.9.0** | **A port published twice by the stack passed the preflight.** The check probed the host, where nothing listens yet. Reproduced by adding Jellyfin to a Silo stack: the web wizard restored Silo's 8096 over the shift already computed. |
| **0.9.0** | **A refused request arrived as a network drop.** The local servers answered without reading the body, and Windows cut the connection: "[WinError 10053]" instead of the 400, "Failed to fetch" in the browser. **Found by looking for why the suite failed one run in three**, on a different test each time. |
| **0.8.0** | **A desynchronised incoming port was reported and never fixed.** `doctor` said "the VPN opened 48406, the client listens on 45270", and the user was left with the problem and without the remedy. The case exists because setting the port is an **event**: Gluetun calls `VPN_PORT_FORWARDING_UP_COMMAND` when it obtains a port — verified on 2026-09-10, called one second after the attribution. A client recreated **between two attributions** therefore gets no call at all. `doctor` now replays the very script Gluetun runs — not a second implementation, which would end up diverging — then **reads back** before concluding. Idea taken from [brumatheus's proposal](https://github.com/yannickuhrig1/plugarr/issues/1), whose periodic poll covered that case. |
| **0.8.0** | **A mistyped VPN provider crashed the executable.** `--vpn-provider zorglub` surfaced a bare `ValidationError`: a pydantic traceback, a link to `errors.pydantic.dev`, and a last line reading "Failed to execute script 'launcher'" — announcing a crash to someone who merely made a typo. The useful sentence was already written by the validator — "fournisseur VPN inconnu de Gluetun … Choix possibles: …" — it was only drowned. **Found by exercising the 0.8.0 executable before publishing it**, not by re-reading the code. |
| **0.8.0** | **The screenshots depended on the machine that produced them.** The wizard looks for a previous installation, in the launch directory and in the user's registry: the summary generated here therefore showed the local path of a real installation — which went straight into a published SVG — and the carry-over also took its timezone from it. CI, which regenerates the screenshots and compares them against the repository, could not match. `freeze_environment()` now neutralises that lookup, and a test refuses any screenshot carrying a trace of the machine that made it. Verified: Windows and Linux now produce byte-identical files. |
