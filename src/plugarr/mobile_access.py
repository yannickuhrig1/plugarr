...
def append_to(page, cfg, host, *, remote=None, demo=False):
    ...
    services = [{"id": sid, "name": catalog.get(sid).display_name,
                ...
                for sid, inst in cfg.services.items() if sid in (*remote_access.SUPPORTED, "prowlarr", "seerr")]
    if not services:
        return page
