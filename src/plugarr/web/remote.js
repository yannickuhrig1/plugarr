...
globalThis.PlugArrRemote = (() => {
  ...
  function mountMobile(data){
    if(!mounted){
      ...
      const box=el('section');box.id='arr-export';box.hidden=true;
      ...
      const nzbBox=el('section');nzbBox.id='nzb-export';
      const nzbLabel=el('label',text('Profil nzb360 24.4.1 (expérimental)','nzb360 24.4.1 profile (experimental)'));
      ...
      const consent=el('label');const check=el('input');check.type='checkbox';check.id='nzb-export-confirm';
      consent.append(check,el('span',text('J’ai sauvegardé mes réglages nzb360. La restauration de ce ZIP expérimental peut remplacer mes réglages et serveurs.','I backed up my nzb360 settings. Restoring this experimental ZIP may replace settings and servers.')));nzbBox.append(consent);
  ...
  function buildNzb360Export(data,network='local'){
    const {payload,omitted}=buildArrControlExport({...data,services:data.services.filter(s=>ids.includes(s.id))},network);
    const preferences={version:'24.4.1',nzbdrone_server_enabled_preference:false,radarr_server_enabled_preference:false,torrent_server_enabled_preference:false};
    for(const service of payload.services){
      ...
      preferences[`${prefix}_server_local_connectionstring_preference`]='';
      preferences[`${prefix}_server_SSID_preference`]='';
    ...
    return {bytes:zipStored(files),count:payload.services.length,omitted};
  }
  function renderNzbExport(){
    ...
    try{
      const result=buildNzb360Export(current,$('nzb-export-network').value);
      $('nzb-export-download').disabled=!result.count||!$('nzb-export-confirm').checked;
      $('nzb-export-notice').textContent=text(`Expérimental 24.4.1 : ${result.count} application(s). Sonarr, Radarr et qBittorrent uniquement. Un seul réseau, sans bascule automatique. Restauration Android non validée.`, `Experimental 24.4.1: ${result.count} service(s). Sonarr, Radarr and qBittorrent only. One network, no automatic switching. Android restore not validated.`)
        +(result.omitted.length?' '+text('Exclues : ','Excluded: ')+result.omitted.join(', '):'')
        +(current.demo?' '+text('DÉMONSTRATION : accès fictifs.','DEMO: fictitious connections.'):'')
        +($('nzb-export-network').value==='remote'?' '+text('Avec Tailscale, connectez aussi le téléphone.','With Tailscale, connect the phone too.'):'');
  ...
  function downloadNzb360(){
    ...
    try{
      const network=$('nzb-export-network').value,result=buildNzb360Export(current,network);if(!result.count)return;
  ...
  function mobile(){
    ...
    renderArrExport();
    renderNzbExport();
    const services=current.services.filter(s=>(client==='arrcontrol'?Object.hasOwn(arrServices,s.id):ids.includes(s.id))&&(client!=='qbremote'||s.id==='qbittorrent'));
  ...
  return {english,init,refresh,read,valid,report,mountMobile,buildArrControlExport,buildNzb360Export};
})();
