...
def test_sabnzbd_direct_n_est_pas_dessine_derriere_gluetun():
    cfg = config('qbittorrent', 'sabnzbd')
    cfg.vpn.enabled = True

    assert pairs(connections.topology(cfg), 'vpn') == {('qbittorrent', 'gluetun')}
...
def test_optional_integrations_follow_the_wiring_conditions():
    ...
    assert pairs(data, 'dispatch') == {
        ('autobrr', sid) for sid in ('sonarr', 'radarr', 'lidarr', 'qbittorrent')
    }
    assert ('autobrr', 'sabnzbd') not in pairs(data, 'dispatch')
...
def test_embedded_icons_are_complete_and_self_contained():
    icons = json.loads((Path(connection_map.__file__).parent / 'data' / 'connection_icons.json').read_text(encoding='utf-8'))
...
def test_console_embeds_shared_renderer_and_admin_adapter():
    ...
    shared = (Path(connection_map.__file__).parent / 'web' / 'graph.js').read_text(encoding='utf-8')
