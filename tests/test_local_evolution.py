...
def test_console_still_answers_while_a_backup_runs(live, monkeypatch, tmp_path):
    """Measured failure: the console froze for the whole duration of a backup.

    `do_GET` and `do_POST` both ran under `maintenance.lock`, and `backup()`
    held that same lock while writing the archive. A `/api/status` issued during
    a backup waited for it to finish: eight seconds on a test stack, minutes on
...
def test_a_backup_does_not_raise_alerts_about_the_stack_it_stopped(live, monkeypatch, tmp_path):
    """Measured on a real stack: one backup produced six false alerts.

    A cold backup stops the containers on purpose. The status poll saw them
    down and fired `Transmission indisponible`, `Sonarr indisponible` and four
    more, then cleared them five seconds later. On a real CONFIG_ROOT the
    backup lasts minutes, so the console shows six red alerts for the whole
    duration of a perfectly normal operation, and the history keeps twelve
    misleading events per backup.
    """
    import time
    ...
    def slow(*_args, **_kwargs):
        started.set()
        time.sleep(2)
        return SimpleNamespace(archive=archive, fichiers=1, volumes=[], mega=0.1, arret=True)

    monkeypatch.setattr('plugarr.maintenance.sauvegarde.sauvegarder', slow)
    monkeypatch.setattr('plugarr.admin.status_payload', lambda *_: {
        'services': [{'id': 'sonarr', 'name': 'Sonarr', 'up': False}], 'engine_available': True})

    worker = threading.Thread(target=lambda: client.post('/api/backup', timeout=30))
    worker.start()
    assert started.wait(10)
    reponse = client.get('/api/status', timeout=10).json()
    worker.join(timeout=30)

    assert reponse['services'][0]['intentional_stop'], "l'arret volontaire n'est pas signale"
    alerte = maintenance.snapshot()['alerts'].get('services:sonarr')
    assert not (alerte and alerte['active']), 'la console a sonne l alarme sur sa propre sauvegarde'
...
def test_frozen_web_launcher_also_checks_and_stages_update(monkeypatch):
    """Les arguments des lanceurs web ne doivent plus contourner GitHub."""
    calls = []
    monkeypatch.setattr(selfupdate.sys, 'platform', 'win32')
    monkeypatch.setattr(selfupdate.sys, 'frozen', True, raising=False)
    monkeypatch.setattr(selfupdate.sys, 'argv', ['plugarr.exe', 'web'])
    ...
    monkeypatch.setattr(selfupdate, 'stage', lambda info: calls.append(info['latest']))
...
def test_backup_failure_does_not_prune(cfg, tmp_path, monkeypatch):
    ...
    assert maintenance.snapshot()['last_backup_error'] == 'OSError : test failure'
